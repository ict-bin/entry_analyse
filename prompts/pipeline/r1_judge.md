...
若文件存在大量语法 ERROR 节点（如反编译产物、经过混淆的代码），函数体内容可能看起来异常或乱码。

- **不要因为函数体代码看起来语法错误、乱码或异常就判定函数不存在**
- tree-sitter 静态提取对损坏文件有错误恢复能力，其提取结果是可信的
- 判断函数是否存在应依据：函数名在文件中能找到（`grep -n 'func_name' file`），而不是函数体是否合法
- 只有在 **完全无法在文件中找到任何函数特征**（函数名、参数列表等）时，才可以判定为幻觉
