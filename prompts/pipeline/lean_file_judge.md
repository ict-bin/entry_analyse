...
**你的目标是把“看起来像入口、但其实只是内部函数”的结果过滤掉。**

- ✅ **要做**：发现格式错误、字段非法、明显错误分类、明显的“内部函数误报成入口”
...
| 误报类型 | 判断依据 | 处理 |
|---------|---------|------|
| 函数名是明显的输出/释放操作 | 函数名含 `send`/`reply`/`resp`/`response`/`print`/`log`/`free`/`destroy`/`dump`/`write` | FAIL，这些不是输入入口 |
| 函数名是明显内部 helper | 函数名含 `fill`/`build`/`convert`/`copy`/`validate`/`set`/`get`/`cleanup`，且无 `handle/dispatch/on/recv` 边界语义 | FAIL |
| 同文件条目过多像“全都算入口” | 同一文件出现大量相似 helper（如 parse/fill/build 一串） | FAIL，提示 Worker 只保留最外层入口 |
| taints 格式非法 | 含中文、空格、括号、`.`、`->` 等非变量字符 | FAIL |
| tag 字段缺失或非 P/A | `tag` 不是 `"P"` 或 `"A"` | FAIL |
| 命中率极端异常 | 条目数 / 函数总数 > 60%（正则明显过宽）| FAIL，要求收缩到真正边界入口 |
...
5. 抽查前 3 条函数名具备明显边界语义（如 handle/dispatch/on/recv/serve/process_msg），且不含 helper/output 语义
