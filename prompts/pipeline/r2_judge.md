...
- 若 grep **无结果**：函数不存在（可能是宏定义或完全虚构）→ 直接输出 `通过: 删除`（见下方格式）
- 若 grep 有结果：继续验证行号准确性
...
```
通过: 删除
反馈: {func_name} 在源文件中不存在，可能是 #define 宏块或 typedef，应从 funcdb 删除
```

## 审核原则

- 必须实际运行 bash sed/grep 命令，不能仅凭印象判断
- 宏定义（`#define`）、`typedef`、全局变量进入 funcdb 是错误的，应输出 `通过: 删除`
- 每次评审只针对指定的单个函数
