"""
entry_analyse — SuperFast 极速流水线引擎（从 engine.py 派生）

与 PipelineEngine 的区别：
  - 无 Judge 循环：R2/R3/R4 仅保留脚本格式校验，移除 Agent Judge
  - 批分类预筛：R2 后插入快速模式批分类（FastModeBatchProcessor）
  - 无 R5：跳过单函数报告生成
  - 本地磁盘：所有中间产物写入 /tmp，任务结束 move 到 NFS
  - 完全独立文件，可随时删除
"""
...
class SuperFastPipelineEngine:
    ...
    async def run(
        ...
        self.cfg.report_func_max_rounds = 0  # R5: 跳过
        self.cfg.report_final_max_rounds = 0
        ...
        def _move_to_nfs() -> None:
            """Move local output/run to NFS, cleanup tmp."""
            try:
                if _nfs_out:
                    _nfs_out.mkdir(parents=True, exist_ok=True)
                _nfs_run.mkdir(parents=True, exist_ok=True)
                # Sync events one last time
                _sync_evt()
                # Move output
                if _local_out.exists():
                    for f in _local_out.iterdir():
                        dst = (_nfs_out or _nfs_run) / f.name
                        try:
                            if f.is_file():
                                _shutil.move(str(f), str(dst))
                            elif f.is_dir() and not dst.exists():
                                _shutil.copytree(str(f), str(dst))
                        except Exception:
                            pass
                # Move run essentials
        ...
        # ── 快速模式：线程安全的流式批处理 ──────────────────────────────
        fm: FastModeBatchProcessor | None = None
        ...
        async def _func_pipeline(
            ...
            if (
                ...
                try:
                    if _r3_entry_path.exists():
                        _r4_entry = await run_in_script_thread(lambda: json.loads(_r3_entry_path.read_text(encoding="utf-8")))
                    else:
                        _r4_entry = {
                            "func_hash":  func_hash,
                    ...
                    if _r4_quick_keep:
                        ...
                        try:
                            ...
                            await run_in_script_thread(lambda: _FDB4.open(dirs.r1, file_hash).update_r4_decision(func_hash, "keep"))
                        except Exception:
                            pass
            ...
            if (out_dir and func_state and
                ...
                try:
                    _entry = await run_in_script_thread(lambda: json.loads(_entry_path.read_text(encoding="utf-8")))
                except Exception:
                    _entry = {
                        "func_hash": func_hash,
                        "function":  func_state.name or "",
    ...
    async def _run_r2(
        ...
        while _should_continue(func_state.r2_j_attempts, r2_max, self._cancel):
            ...
            try:
                ...
                updated = await run_in_script_thread(lambda: _FuncDB.open(dirs.r1, file_hash).get_function(func_hash))
                if updated:
    ...
    async def _run_r3_analysis(
        ...
        if state.cc_state == NodeState.PASSED and func_state.has_external_input:
            try:
                ...
                _fn3 = await run_in_script_thread(lambda: _FDB_CC.open(dirs.r1, file_hash).get_function(func_hash))
...
# ── R2（ctags 行号修正）+ R3（外部输入分析）W+J（每函数串链）───────────────
    ...
    async def _run_r2_j(
        ...
        try:
            ...
            _script_verdict, _script_reason = await run_in_script_thread(
                    _r2_j_script_check,
            ...
            if delete_verdict:
                ...
                try:
                    ...
                    await run_in_script_thread(FunctionDB.open(dirs.r1, file_hash).delete_function, func_hash)
    ...
    async def _run_r4_for_func(
        ...
        if result_file.exists():
            try:
                d = await run_in_script_thread(lambda: json.loads(result_file.read_text(encoding="utf-8")))
    ...
    async def _run_report_for_func(
        ...
        while _should_continue(attempts, report_func_max, self._cancel):
            ...
            write_stage_result_files(
                ...
                raw_text=(await run_in_script_thread(lambda: report_out.read_text(encoding="utf-8")) if report_out.exists() else ""),
            )
