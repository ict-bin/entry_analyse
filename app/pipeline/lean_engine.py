...
"""
entry_analyse — 精简模式（Lean Mode）流水线引擎（engine.py 副本 + API_Filter）

本文件是 engine.py（完整模式）的副本，在 R2 与 R3 之间插入了
API_Filter 阶段：直接调用 LLM API（无 pi 子进程）快速预筛，
只有被判定为外部入口的函数才进入 R3 完整 Agent 分析。

class LeanPipelineEngine  ←  orchestrator.py lean_mode=True 时使用
"""
"""
